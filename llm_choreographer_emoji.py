...
class LLMChoreographer:
    """LLM编舞生成器，使用GPT生成舞蹈动作建议 - 兼容OpenAI 0.28.0"""
    ...
    def __init__(self):
        ...
        print(f"🔧 LLMChoreographer初始化完成，使用OpenAI 0.28.0 API")
    
    def _call_openai(self, messages, max_tokens=100, temperature=0.7):
        """统一的OpenAI API调用方法 - 旧版本API"""
        print(f"📞 调用OpenAI API (v0.28.0)，消息数量: {len(messages)}")
