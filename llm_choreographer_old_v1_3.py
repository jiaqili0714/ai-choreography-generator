...
class LLMChoreographer:
    ...
    def __init__(self):
        ...
        # 直接使用旧版本API，避免新版本的兼容性问题
        openai.api_key = config.OPENAI_API_KEY
        self.use_new_api = False
        print(f"🔧 LLMChoreographer初始化完成，API密钥: {config.OPENAI_API_KEY[:10]}...")
    ...
    def _call_openai(self, messages, max_tokens=100, temperature=0.7):
        ...
        try:
            ...
            print(f"✅ 新版本API调用成功，响应长度: {len(result)}")
            return result
            ...
            try:
                ...
                result = response.choices[0].message.content.strip()
                print(f"✅ 旧版本API调用成功，响应长度: {len(result)}")
                return result
    ...
    def generate_segment_choreography(self, segment: Dict[str, Any], bpm: float, 
        ...
        try:
            content = self._call_openai([{"role": "user", "content": prompt}], max_tokens=300, temperature=0.8)
            print(f"📝 API返回内容: {content[:100]}...")
            
            # 尝试解析JSON响应
            try:
                choreography = json.loads(content)
                print(f"✅ JSON解析成功: {choreography}")
                return choreography
    ...
    def generate_full_choreography(self, segments: List[Dict[str, Any]], 
        ...
        try:
            summary = self._call_openai([{"role": "user", "content": summary_prompt}], max_tokens=200)
            print(f"📋 编舞总结: {summary}")
        except Exception as e:
            print(f"❌ 生成编舞总结时出错: {e}")
